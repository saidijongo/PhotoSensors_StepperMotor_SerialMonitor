    ...
    if (!resetInProgress && !moveInProgress) {
      if (command == "C") {
        if (!leftSensorInterrupted) {
          ...
          motorStopped = false;
          moveInProgress = true;
          rotate(CCW, int(8 / angleToSteps), true); // Rotate 8 degrees counterclockwise
          stopMotor();
          Serial.println("CCW 8 degrees");
        }
      } else if (command == "D") {
        if (!rightSensorInterrupted) {
          motorStopped = false;
          moveInProgress = true;
          rotate(CCW, int(90 / angleToSteps), true); // Rotate 90 degrees counterclockwise
          moveInProgress = false;
          motorStopped = true;
        } else {
          motorStopped = false;
          moveInProgress = true;
          rotate(CW, int(8 / angleToSteps), true); // Rotate 8 degrees clockwise
          stopMotor();
          Serial.println("CW 8 degrees");
        }
      } else if (command == "R") {
